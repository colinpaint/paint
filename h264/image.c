...
//}}}
//{{{
static void setup_buffers (VideoParameters* pVid, int layer_id) {


  if (pVid->last_dec_layer_id != layer_id) {
    CodingParameters* cps = pVid->p_EncodePar[layer_id];
    if (cps->separate_colour_plane_flag) {
      for (int i = 0; i < MAX_PLANE; i++ ) {
        pVid->mb_data_JV[i] = cps->mb_data_JV[i];
        pVid->intra_block_JV[i] = cps->intra_block_JV[i];
        pVid->ipredmode_JV[i] = cps->ipredmode_JV[i];
        pVid->siblock_JV[i] = cps->siblock_JV[i];
        }
      pVid->mb_data = NULL;
      pVid->intra_block = NULL;
      pVid->ipredmode = NULL;
      pVid->siblock = NULL;
      }
    else {
      pVid->mb_data = cps->mb_data;
      pVid->intra_block = cps->intra_block;
      pVid->ipredmode = cps->ipredmode;
      pVid->siblock = cps->siblock;
      }

    pVid->PicPos = cps->PicPos;
    pVid->nz_coeff = cps->nz_coeff;
    pVid->qp_per_matrix = cps->qp_per_matrix;
    pVid->qp_rem_matrix = cps->qp_rem_matrix;
    pVid->oldFrameSizeInMbs = cps->oldFrameSizeInMbs;
    pVid->last_dec_layer_id = layer_id;
    }
  }
//}}}
...
//}}}
//{{{
static void mbAffPostProc (VideoParameters* pVid) {

  imgpel temp_buffer[32][16];

  StorablePicture* dec_picture = pVid->dec_picture;
  ...
  for (short i = 0; i < (int)dec_picture->PicSizeInMbs; i += 2) {
    if (dec_picture->motion.mb_field[i]) {
      get_mb_pos (pVid, i, pVid->mb_size[IS_LUMA], &x0, &y0);
      update_mbaff_macroblock_data (imgY + y0, temp_buffer, x0, MB_BLOCK_SIZE, MB_BLOCK_SIZE);

      if (dec_picture->chroma_format_idc != YUV400) {
        x0 = (short)((x0 * pVid->mb_cr_size_x) >> 4);
        y0 = (short)((y0 * pVid->mb_cr_size_y) >> 4);
        update_mbaff_macroblock_data (imgUV[0] + y0, temp_buffer, x0, pVid->mb_cr_size_x, pVid->mb_cr_size_y);
        update_mbaff_macroblock_data (imgUV[1] + y0, temp_buffer, x0, pVid->mb_cr_size_x, pVid->mb_cr_size_y);
        }
      }
    }
  }
...
//}}}
//{{{
static void errorTracking (VideoParameters* pVid, Slice *currSlice) {

  if (currSlice->redundant_pic_cnt == 0)
    pVid->Is_primary_correct = pVid->Is_redundant_correct = 1;

  if (currSlice->redundant_pic_cnt == 0 && pVid->type != I_SLICE) {
    for (int i = 0; i < currSlice->num_ref_idx_active[LIST_0];++i)
      if (currSlice->ref_flag[i] == 0)  // any reference of primary slice is incorrect
        pVid->Is_primary_correct = 0; // primary slice is incorrect
    }
  else if (currSlice->redundant_pic_cnt != 0 && pVid->type != I_SLICE)
...
static void reorderLists (Slice* currSlice) {
  ...
  if ((currSlice->slice_type != I_SLICE)&&(currSlice->slice_type != SI_SLICE)) {
    ...
    if (pVid->no_reference_picture == currSlice->listX[0][currSlice->num_ref_idx_active[LIST_0] - 1]) {
      if (pVid->non_conforming_stream)
  ...
  if (currSlice->slice_type == B_SLICE) {
    ...
    if (pVid->no_reference_picture == currSlice->listX[1][currSlice->num_ref_idx_active[LIST_1]-1]) {
      if (pVid->non_conforming_stream)
...
static void ercWriteMBMODEandMV (Macroblock* currMB) {
  ...
  int currMBNum = currMB->mbAddrX; //pVid->currentSlice->current_mb_nr;
  StorablePicture* dec_picture = pVid->dec_picture;
  int mbx = xPosMB(currMBNum, dec_picture->size_x), mby = yPosMB(currMBNum, dec_picture->size_x);

  objectBuffer_t* currRegion = pVid->erc_object_list + (currMBNum<<2);

  if (pVid->type != B_SLICE) {
    //{{{  non-B frame
    for (int i = 0; i < 4; ++i) {
...
//}}}
//{{{
static void init_cur_imgy (Slice* currSlice, VideoParameters* pVid) {

  if ((pVid->separate_colour_plane_flag != 0)) {
    StorablePicture* vidref = pVid->no_reference_picture;
    int noref = (currSlice->framepoc < pVid->recovery_poc);
...
static int isNewPicture (StorablePicture* dec_picture, Slice* currSlice, OldSliceParams* p_old_slice) {
  ...
  VideoParameters* pVid = currSlice->pVid;
  if (pVid->active_sps->pic_order_cnt_type == 0) {
    result |= (p_old_slice->pic_oder_cnt_lsb != currSlice->pic_order_cnt_lsb);
    if (pVid->active_pps->bottom_field_pic_order_in_frame_present_flag  ==  1 &&  !currSlice->field_pic_flag )
      result |= (p_old_slice->delta_pic_oder_cnt_bottom != currSlice->delta_pic_order_cnt_bottom);
    }

  if (pVid->active_sps->pic_order_cnt_type == 1) {
    if (!pVid->active_sps->delta_pic_order_always_zero_flag) {
...
//}}}
//{{{
static void copyDecPicture_JV (VideoParameters* pVid, StorablePicture* dst, StorablePicture* src ) {
...
//}}}
//{{{
static void initPicture (VideoParameters* pVid, Slice *currSlice, InputParameters *p_Inp) {
  ...
  pVid->PicHeightInMbs = pVid->FrameHeightInMbs / ( 1 + currSlice->field_pic_flag );
  pVid->PicSizeInMbs = pVid->PicWidthInMbs * pVid->PicHeightInMbs;
  pVid->FrameSizeInMbs = pVid->PicWidthInMbs * pVid->FrameHeightInMbs;
  ...
  setup_buffers (pVid, currSlice->layer_id);

  if (pVid->recovery_point)
    pVid->recovery_frame_num = (currSlice->frame_num + pVid->recovery_frame_cnt) % pVid->max_frame_num;
  if (currSlice->idr_flag)
    pVid->recovery_frame_num = currSlice->frame_num;
  if (pVid->recovery_point == 0 &&
    currSlice->frame_num != pVid->pre_frame_num &&
    currSlice->frame_num != (pVid->pre_frame_num + 1) % pVid->max_frame_num) {
    if (active_sps->gaps_in_frame_num_value_allowed_flag == 0) {
      // picture error concealment
      if (p_Inp->conceal_mode !=0) {
        if ((currSlice->frame_num) < ((pVid->pre_frame_num + 1) % pVid->max_frame_num)) {
          ...
          pVid->conceal_mode = 1;
          pVid->IDR_concealment_flag = 1;
          conceal_lost_frames(p_Dpb, currSlice);
          // reset to original concealment mode for future drops
          pVid->conceal_mode = p_Inp->conceal_mode;
          }
        else {
          // reset to original concealment mode for future drops
          pVid->conceal_mode = p_Inp->conceal_mode;
          pVid->IDR_concealment_flag = 0;
          conceal_lost_frames(p_Dpb, currSlice);
          }
        }
      else
    ...
    if (pVid->conceal_mode == 0)
      fill_frame_num_gap(pVid, currSlice);
    }

  if (currSlice->nal_reference_idc)
    pVid->pre_frame_num = currSlice->frame_num;

  // calculate POC
  decodePOC (pVid, currSlice);

  if (pVid->recovery_frame_num == (int) currSlice->frame_num && pVid->recovery_poc == 0x7fffffff)
    pVid->recovery_poc = currSlice->framepoc;

  if(currSlice->nal_reference_idc)
    pVid->last_ref_pic_poc = currSlice->framepoc;

  if (currSlice->structure == FRAME || currSlice->structure == TOP_FIELD)
    gettime (&(pVid->start_time));

  dec_picture = pVid->dec_picture = alloc_storable_picture (pVid, currSlice->structure, pVid->width, pVid->height, pVid->width_cr, pVid->height_cr, 1);
  ...
  dec_picture->chroma_qp_offset[0] = pVid->active_pps->chroma_qp_index_offset;
  dec_picture->chroma_qp_offset[1] = pVid->active_pps->second_chroma_qp_index_offset;
  ...
  ercReset (pVid->erc_errorVar, pVid->PicSizeInMbs, pVid->PicSizeInMbs, dec_picture->size_x);
  ...
  switch (currSlice->structure ) {
    ...
    //{{{
    default:
      error ("pVid->structure not initialized", 235);
    //}}}
    }

  //pVid->current_slice_nr=0;
  if (pVid->type > SI_SLICE) {
    set_ec_flag (pVid, SE_PTYPE);
    pVid->type = P_SLICE;  // concealed element
    }

  // CAVLC init
  if (pVid->active_pps->entropy_coding_mode_flag == (Boolean) CAVLC)
    memset (pVid->nz_coeff[0][0][0], -1, pVid->PicSizeInMbs * 48 *sizeof(byte)); // 3 * 4 * 4
  ...
  if (pVid->separate_colour_plane_flag != 0) {
    for (int nplane = 0; nplane < MAX_PLANE; ++nplane ) {
      Macroblock* currMB = pVid->mb_data_JV[nplane];
      char* intra_block = pVid->intra_block_JV[nplane];
      for (int i = 0; i < (int)pVid->PicSizeInMbs; ++i)
        reset_mbs (currMB++);
      memset (pVid->ipredmode_JV[nplane][0], DC_PRED, 16 * pVid->FrameHeightInMbs * pVid->PicWidthInMbs * sizeof(char));
    ...
    Macroblock* currMB = pVid->mb_data;
    for (int i = 0; i < (int)pVid->PicSizeInMbs; ++i)
      reset_mbs (currMB++);
    if (pVid->active_pps->constrained_intra_pred_flag)
      for (int i = 0; i < (int)pVid->PicSizeInMbs; ++i)
        pVid->intra_block[i] = 1;
    memset (pVid->ipredmode[0], DC_PRED, 16 * pVid->FrameHeightInMbs * pVid->PicWidthInMbs * sizeof(char));
    }

  dec_picture->slice_type = pVid->type;
  ...
  dec_picture->PicWidthInMbs = pVid->PicWidthInMbs;

  pVid->get_mb_block_pos = dec_picture->mb_aff_frame_flag ? get_mb_block_pos_mbaff : get_mb_block_pos_normal;
  pVid->getNeighbour = dec_picture->mb_aff_frame_flag ? getAffNeighbour : getNonAffNeighbour;

  dec_picture->pic_num = currSlice->frame_num;
  dec_picture->frame_num = currSlice->frame_num;
  dec_picture->recovery_frame = (unsigned int)((int)currSlice->frame_num == pVid->recovery_frame_num);
  ...
  if (pVid->separate_colour_plane_flag != 0) {
    pVid->dec_picture_JV[0] = pVid->dec_picture;
    pVid->dec_picture_JV[1] = alloc_storable_picture (pVid, (PictureStructure) currSlice->structure, pVid->width, pVid->height, pVid->width_cr, pVid->height_cr, 1);
    copyDecPicture_JV (pVid, pVid->dec_picture_JV[1], pVid->dec_picture_JV[0] );
    pVid->dec_picture_JV[2] = alloc_storable_picture (pVid, (PictureStructure) currSlice->structure, pVid->width, pVid->height, pVid->width_cr, pVid->height_cr, 1);
    copyDecPicture_JV (pVid, pVid->dec_picture_JV[2], pVid->dec_picture_JV[0] );
    }
  }
//}}}
//{{{
static void initPictureDecoding (VideoParameters* pVid) {

  int iDeblockMode = 1;

  if (pVid->iSliceNumOfCurrPic >= MAX_NUM_SLICES)
    error ("Maximum number of supported slices exceeded, increase MAX_NUM_SLICES", 200);

  Slice* slice = pVid->ppSliceList[0];
  if (pVid->pNextPPS->Valid &&
      (int)pVid->pNextPPS->pic_parameter_set_id == slice->pic_parameter_set_id) {
    pic_parameter_set_rbsp_t tmpPPS;
    memcpy (&tmpPPS, &(pVid->PicParSet[slice->pic_parameter_set_id]), sizeof (pic_parameter_set_rbsp_t));
    (pVid->PicParSet[slice->pic_parameter_set_id]).slice_group_id = NULL;
    MakePPSavailable (pVid, pVid->pNextPPS->pic_parameter_set_id, pVid->pNextPPS);
    memcpy (pVid->pNextPPS, &tmpPPS, sizeof (pic_parameter_set_rbsp_t));
  ...
  pVid->PicHeightInMbs = pVid->FrameHeightInMbs / ( 1 + slice->field_pic_flag );
  pVid->PicSizeInMbs = pVid->PicWidthInMbs * pVid->PicHeightInMbs;
  pVid->FrameSizeInMbs = pVid->PicWidthInMbs * pVid->FrameHeightInMbs;
  pVid->structure = slice->structure;
  fmo_init (pVid, slice);

  update_pic_num (slice);

  init_Deblock (pVid, slice->mb_aff_frame_flag);
  for (int j = 0; j < pVid->iSliceNumOfCurrPic; j++) {
    if (pVid->ppSliceList[j]->DFDisableIdc != 1)
      iDeblockMode = 0;
    }

  pVid->iDeblockMode = iDeblockMode;
  }
//}}}
static void framePostProcessing (VideoParameters* pVid) {}
static void fieldPostProcessing (VideoParameters* pVid) { pVid->number /= 2; }
...
static void copySliceInfo (Slice* currSlice, OldSliceParams* p_old_slice) {
  ...
  p_old_slice->frame_num = currSlice->frame_num; //pVid->frame_num;
  p_old_slice->field_pic_flag = currSlice->field_pic_flag; //pVid->field_pic_flag;
...
//}}}
//{{{
static void initSlice (VideoParameters* pVid, Slice *currSlice) {

  pVid->active_sps = currSlice->active_sps;
  pVid->active_pps = currSlice->active_pps;
  ...
  if (currSlice->structure == FRAME)
    init_mbaff_lists (pVid, currSlice);

  // update reference flags and set current pVid->ref_flag
  if (!(currSlice->redundant_pic_cnt != 0 && pVid->previous_frame_num == currSlice->frame_num))
    for (int i = 16; i > 0;i--)
      currSlice->ref_flag[i] = currSlice->ref_flag[i-1];
  currSlice->ref_flag[0] = currSlice->redundant_pic_cnt == 0 ? pVid->Is_primary_correct :
                                                               pVid->Is_redundant_correct;
...
static void decodeOneSlice (Slice* currSlice) {
  ...
  VideoParameters* pVid = currSlice->pVid;
  if ((pVid->separate_colour_plane_flag != 0))
    change_plane_JV (pVid, currSlice->colour_plane_id, currSlice );
  else {
    currSlice->mb_data = pVid->mb_data;
    currSlice->dec_picture = pVid->dec_picture;
    currSlice->siblock = pVid->siblock;
    currSlice->ipredmode = pVid->ipredmode;
    currSlice->intra_block = pVid->intra_block;
    }
...
static int readNewSlice (Slice* currSlice) {
  ...
  for (;;) {
    NALU_t* nalu = pVid->nalu;
    if (!pendingNalu) {
      if (!readNextNalu (pVid, nalu))
        return EOS;
      }
    else {
      nalu = pendingNalu;
  ...
  process_nalu:
    switch (nalu->nal_unit_type) {
      ...
      //{{{
      case NALU_TYPE_IDR:
        if (pVid->recovery_point || nalu->nal_unit_type == NALU_TYPE_IDR) {
          if (pVid->recovery_point_found == 0) {
            if (nalu->nal_unit_type != NALU_TYPE_IDR) {
              printf ("Warning: Decoding does not start with an IDR picture.\n");
              pVid->non_conforming_stream = 1;
              }
            else
              pVid->non_conforming_stream = 0;
            }
          pVid->recovery_point_found = 1;
          }
        ...
        currSlice->active_sps = pVid->active_sps;
        currSlice->active_pps = pVid->active_pps;
        currSlice->Transform8x8Mode = pVid->active_pps->transform_8x8_mode_flag;
        currSlice->chroma444_not_separate = (pVid->active_sps->chroma_format_idc == YUV444) &&
                                            (pVid->separate_colour_plane_flag == 0);
        RestOfSliceHeader (currSlice);
        assign_quant_params (currSlice);

        // if primary slice is replaced with redundant slice, set the correct image type
        if (currSlice->redundant_pic_cnt &&
            pVid->Is_primary_correct == 0 &&
            pVid->Is_redundant_correct)
          pVid->dec_picture->slice_type = pVid->type;

        if (isNewPicture (pVid->dec_picture, currSlice, pVid->old_slice)) {
          if (pVid->iSliceNumOfCurrPic == 0)
            initPicture (pVid, currSlice, p_Inp);
          current_header = SOP;
          // check zero_byte if it is also the first NAL unit in the access unit
          checkZeroByteVCL (pVid, nalu);
          }
        else
          current_header = SOS;
      ...
      case NALU_TYPE_DPA:
        ...
        // From here on, pVid->active_sps, pVid->active_pps and the slice header are valid
        if (currSlice->mb_aff_frame_flag)
          currSlice->current_mb_nr = currSlice->start_mb_nr << 1;
        else
          currSlice->current_mb_nr = currSlice->start_mb_nr;
        ...
        if (NALU_TYPE_DPB == nalu->nal_unit_type) {
          ...
          if ((slice_id_b != slice_id_a) || (nalu->lost_packets)) {
            ...
            if (pVid->active_pps->redundant_pic_cnt_present_flag)
      ...
      //{{{
      case NALU_TYPE_SEI:
        InterpretSEIMessage (nalu->buf,nalu->len,pVid, currSlice);
        break;
      //}}}
      //{{{
      case NALU_TYPE_PPS:
        ProcessPPS (pVid, nalu);
        break;
      //}}}
      //{{{
      case NALU_TYPE_SPS:
        ProcessSPS (pVid, nalu);
        break;
      //}}}
...
//}}}
//{{{
void pad_dec_picture (VideoParameters* pVid, StorablePicture* dec_picture)
{
  int iPadX = pVid->iLumaPadX;
  int iPadY = pVid->iLumaPadY;
  ...
  if (dec_picture->chroma_format_idc != YUV400) {
    iPadX = pVid->iChromaPadX;
    iPadY = pVid->iChromaPadY;
...
//}}}
//{{{
void calculate_frame_no (VideoParameters* pVid, StorablePicture *p) {

  InputParameters* p_Inp = pVid->p_Inp;

  // calculate frame number
  int psnrPOC = pVid->active_sps->mb_adaptive_frame_field_flag ? p->poc / (p_Inp->poc_scale) :
                                                                  p->poc / (p_Inp->poc_scale);
  if (psnrPOC == 0)
    pVid->idr_psnr_number = pVid->g_nFrame * pVid->ref_poc_gap/(p_Inp->poc_scale);
  pVid->psnr_number = imax(pVid->psnr_number, pVid->idr_psnr_number+psnrPOC);

  pVid->frame_no = pVid->idr_psnr_number + psnrPOC;
  }
//}}}
//{{{
void exit_picture (VideoParameters* pVid, StorablePicture** dec_picture) {

  InputParameters* p_Inp = pVid->p_Inp;

  // return if the last picture has already been finished
  if (*dec_picture == NULL ||
      (pVid->num_dec_mb != pVid->PicSizeInMbs &&
       (pVid->yuv_format != YUV444 || !pVid->separate_colour_plane_flag)))
    return;
  ...
  if (!(*dec_picture)->mb_aff_frame_flag) {
    int i;
    ercStartSegment (0, ercSegment, 0 , pVid->erc_errorVar);
    // generate the segments according to the macroblock map
    for (i = 1; i < (int) (*dec_picture)->PicSizeInMbs; ++i) {
      if (pVid->mb_data[i].ei_flag != pVid->mb_data[i-1].ei_flag) {
        ercStopSegment (i-1, ercSegment, 0, pVid->erc_errorVar); //! stop current segment

        // mark current segment as lost or OK
        if(pVid->mb_data[i-1].ei_flag)
          ercMarkCurrSegmentLost ((*dec_picture)->size_x, pVid->erc_errorVar);
        else
          ercMarkCurrSegmentOK ((*dec_picture)->size_x, pVid->erc_errorVar);

        ++ercSegment;  //! next segment
        ercStartSegment (i, ercSegment, 0 , pVid->erc_errorVar); //! start new segment
        //ercStartMB = i;//! save start MB for this segment
        }
      }

    // mark end of the last segment
    ercStopSegment ((*dec_picture)->PicSizeInMbs-1, ercSegment, 0, pVid->erc_errorVar);
    if(pVid->mb_data[i-1].ei_flag)
      ercMarkCurrSegmentLost ((*dec_picture)->size_x, pVid->erc_errorVar);
    else
      ercMarkCurrSegmentOK ((*dec_picture)->size_x, pVid->erc_errorVar);
    ...
    pVid->erc_mvperMB /= (*dec_picture)->PicSizeInMbs;
    pVid->erc_img = pVid;
    if ((*dec_picture)->slice_type == I_SLICE || (*dec_picture)->slice_type == SI_SLICE) // I-frame
      ercConcealIntraFrame (pVid, &recfr, (*dec_picture)->size_x, (*dec_picture)->size_y, pVid->erc_errorVar);
    else
      ercConcealInterFrame (&recfr, pVid->erc_object_list, (*dec_picture)->size_x, (*dec_picture)->size_y, pVid->erc_errorVar, (*dec_picture)->chroma_format_idc);
    }
  //}}}
  if (!pVid->iDeblockMode &&
      (pVid->bDeblockEnable & (1<<(*dec_picture)->used_for_reference))) {
    //{{{  deblocking for frame or field
    if( (pVid->separate_colour_plane_flag != 0) ) {
      int nplane;
      int colour_plane_id = pVid->ppSliceList[0]->colour_plane_id;
      for( nplane=0; nplane<MAX_PLANE; ++nplane ) {
        pVid->ppSliceList[0]->colour_plane_id = nplane;
        change_plane_JV( pVid, nplane, NULL );
        DeblockPicture( pVid, *dec_picture );
        }
      pVid->ppSliceList[0]->colour_plane_id = colour_plane_id;
      make_frame_picture_JV(pVid);
      }
    else
      DeblockPicture (pVid, *dec_picture);
    }
    //}}}
  else if (pVid->separate_colour_plane_flag != 0)
    make_frame_picture_JV (pVid);

  if ((*dec_picture)->mb_aff_frame_flag)
    mbAffPostProc (pVid);

  if (pVid->structure == FRAME)
    framePostProcessing (pVid);
  else
    fieldPostProcessing (pVid);

  if ((*dec_picture)->used_for_reference)
    pad_dec_picture (pVid, *dec_picture);
  ...
  store_picture_in_dpb (pVid->p_Dpb_layer[0], *dec_picture);

  *dec_picture = NULL;
  if (pVid->last_has_mmco_5)
    pVid->pre_frame_num = 0;

  if (structure == TOP_FIELD || structure == FRAME) {
    //{{{  frame type string
    if (slice_type == I_SLICE && is_idr) // IDR picture
      strcpy (pVid->cslice_type,"IDR");
    else if (slice_type == I_SLICE) // I picture
      strcpy (pVid->cslice_type," I ");
    else if (slice_type == P_SLICE) // P pictures
      strcpy (pVid->cslice_type," P ");
    else if (slice_type == SP_SLICE) // SP pictures
      strcpy (pVid->cslice_type,"SP ");
    else if  (slice_type == SI_SLICE)
      strcpy (pVid->cslice_type,"SI ");
    else if (refpic) // stored B pictures
      strcpy (pVid->cslice_type," B ");
    else // B pictures
      strcpy (pVid->cslice_type," b ");

    if (structure == FRAME)
      strncat (pVid->cslice_type, "       ",8-strlen(pVid->cslice_type));
    }
    //}}}
  else if (structure == BOTTOM_FIELD) {
    //{{{  frame type string
    if (slice_type == I_SLICE && is_idr) // IDR picture
      strncat (pVid->cslice_type,"|IDR",8-strlen(pVid->cslice_type));
    else if (slice_type == I_SLICE) // I picture
      strncat (pVid->cslice_type,"| I ",8-strlen(pVid->cslice_type));
    else if (slice_type == P_SLICE) // P pictures
      strncat (pVid->cslice_type,"| P ",8-strlen(pVid->cslice_type));
    else if (slice_type == SP_SLICE) // SP pictures
      strncat (pVid->cslice_type,"|SP ",8-strlen(pVid->cslice_type));
    else if  (slice_type == SI_SLICE)
      strncat (pVid->cslice_type,"|SI ",8-strlen(pVid->cslice_type));
    else if (refpic) // stored B pictures
      strncat (pVid->cslice_type,"| B ",8-strlen(pVid->cslice_type));
    else // B pictures
      strncat (pVid->cslice_type,"| b ",8-strlen(pVid->cslice_type));
    }
    //}}}
  pVid->cslice_type[8] = 0;

  if ((structure == FRAME) || structure == BOTTOM_FIELD) {
    gettime (&(pVid->end_time));
    int64 tmp_time = timediff(&(pVid->start_time), &(pVid->end_time));
    pVid->tot_time += tmp_time;
    printf ("%5d %s poc:%4d pic:%3d qp:%2d %dms\n",
            pVid->frame_no, pVid->cslice_type, frame_poc, pic_num, qp, (int)timenorm(tmp_time));
...
int decode_one_frame (DecoderParams* pDecoder) {
  ...
  VideoParameters* pVid = pDecoder->pVid;
  InputParameters* p_Inp = pVid->p_Inp;

  int iSliceNo = 0;
  Slice* currSlice = NULL;
  Slice** ppSliceList = pVid->ppSliceList;

  // read one picture first
  pVid->iSliceNumOfCurrPic = 0;
  pVid->iNumOfSlicesDecoded = 0;
  pVid->num_dec_mb = 0;
  ...
  if (pVid->newframe) {
    if (pVid->pNextPPS->Valid) {
      //{{{  use next PPS
      MakePPSavailable (pVid, pVid->pNextPPS->pic_parameter_set_id, pVid->pNextPPS);
      pVid->pNextPPS->Valid = 0;
      }
      //}}}
    //{{{  get firstSlice from currentSlice;
    assert (ppSliceList[pVid->iSliceNumOfCurrPic]);

    currSlice = ppSliceList[pVid->iSliceNumOfCurrPic];
    ppSliceList[pVid->iSliceNumOfCurrPic] = pVid->pNextSlice;
    pVid->pNextSlice = currSlice;
    assert (ppSliceList[pVid->iSliceNumOfCurrPic]->current_slice_nr == 0);

    currSlice = ppSliceList[pVid->iSliceNumOfCurrPic];
    UseParameterSet (currSlice);
    initPicture (pVid, currSlice, p_Inp);
    pVid->iSliceNumOfCurrPic++;
  ...
  while (current_header != SOP && current_header != EOS) {
    //{{{  no pending slices
    assert (pVid->iSliceNumOfCurrPic < pVid->iNumOfSlicesAllocated);

    if (!ppSliceList[pVid->iSliceNumOfCurrPic])
      ppSliceList[pVid->iSliceNumOfCurrPic] = malloc_slice (p_Inp, pVid);

    currSlice = ppSliceList[pVid->iSliceNumOfCurrPic];
    currSlice->pVid = pVid;
    currSlice->p_Inp = p_Inp;
    currSlice->p_Dpb = pVid->p_Dpb_layer[0]; //set default value;
    ...
    if (currSlice->frame_num == pVid->previous_frame_num &&
    ...
    if ((current_header != SOP && current_header != EOS) ||
        (pVid->iSliceNumOfCurrPic == 0 && current_header == SOP)) {
       currSlice->current_slice_nr = (short)pVid->iSliceNumOfCurrPic;
       pVid->dec_picture->max_slice_id =
         (short)imax (currSlice->current_slice_nr, pVid->dec_picture->max_slice_id);
       if (pVid->iSliceNumOfCurrPic > 0) {
         copyPOC (*ppSliceList, currSlice);
         ppSliceList[pVid->iSliceNumOfCurrPic-1]->end_mb_nr_plus1 = currSlice->start_mb_nr;
         }

       pVid->iSliceNumOfCurrPic++;
       if (pVid->iSliceNumOfCurrPic >= pVid->iNumOfSlicesAllocated) {
         Slice** tmpSliceList = (Slice**)realloc (
           pVid->ppSliceList, (pVid->iNumOfSlicesAllocated+MAX_NUM_DECSLICES)*sizeof(Slice*));
         if (!tmpSliceList) {
           tmpSliceList = calloc ((pVid->iNumOfSlicesAllocated + MAX_NUM_DECSLICES), sizeof(Slice*));
           memcpy (tmpSliceList, pVid->ppSliceList, pVid->iSliceNumOfCurrPic*sizeof(Slice*));
           free (pVid->ppSliceList);
           ppSliceList = pVid->ppSliceList = tmpSliceList;
           }
         else {
           ppSliceList = pVid->ppSliceList = tmpSliceList;
           memset (pVid->ppSliceList + pVid->iSliceNumOfCurrPic, 0, sizeof(Slice*)*MAX_NUM_DECSLICES);
           }
      ...
      if (ppSliceList[pVid->iSliceNumOfCurrPic-1]->mb_aff_frame_flag)
        ppSliceList[pVid->iSliceNumOfCurrPic-1]->end_mb_nr_plus1 = pVid->FrameSizeInMbs/2;
      else
        ppSliceList[pVid->iSliceNumOfCurrPic-1]->end_mb_nr_plus1 = pVid->FrameSizeInMbs/(1+ppSliceList[pVid->iSliceNumOfCurrPic-1]->field_pic_flag);
      pVid->newframe = 1;
    ...
    copySliceInfo (currSlice, pVid->old_slice);
    }
    //}}}

  // decode slices
  ret = current_header;
  initPictureDecoding (pVid);
  for (iSliceNo = 0; iSliceNo < pVid->iSliceNumOfCurrPic; iSliceNo++) {
    ...
    initSlice (pVid, currSlice);
    decodeSlice (currSlice, current_header);
    pVid->iNumOfSlicesDecoded++;

    pVid->num_dec_mb += currSlice->num_dec_mb;
    pVid->erc_mvperMB += currSlice->erc_mvperMB;
    }

  if (pVid->dec_picture->structure == FRAME)
    pVid->last_dec_poc = pVid->dec_picture->frame_poc;
  else if (pVid->dec_picture->structure == TOP_FIELD)
    pVid->last_dec_poc = pVid->dec_picture->top_poc;
  else if (pVid->dec_picture->structure == BOTTOM_FIELD)
    pVid->last_dec_poc = pVid->dec_picture->bottom_poc;

  exit_picture (pVid, &pVid->dec_picture);

  pVid->previous_frame_num = ppSliceList[0]->frame_num;
