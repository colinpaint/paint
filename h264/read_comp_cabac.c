...
static void readCompCoeff8x8_CABAC (sMacroblock* currMB, sSyntaxElement* currSE, sColorPlane pl, int b8)
{
  ...
  {
    sVidParam* vidParam = currMB->vidParam;
    int transform_pl = (vidParam->separate_colour_plane_flag != 0) ? currMB->p_Slice->colour_plane_id : pl;
    ...
    int qp_per = vidParam->qp_per_matrix[ currMB->qp_scaled[pl] ];
    int qp_rem = vidParam->qp_rem_matrix[ currMB->qp_scaled[pl] ];
  ...
  else // read DC coeffs for new intra modes
  {
    ...
    if (!currMB->dpl_flag)
    {
      ...
      if(currMB->is_lossless == FALSE)
        itrans_2(currMB, (sColorPlane) currSlice->colour_plane_id);// transform new intra DC
    }
  }
...
static void read_CBP_and_coeffs_from_NAL_CABAC_444 (sMacroblock* currMB)
{
  ...
  for (uv = 0; uv < 2; ++uv )
  {
    ...
    if (IS_I16MB (currMB)) // read DC coeffs for new intra modes
    {
      ...
      if(currMB->is_lossless == FALSE)
      {
        itrans_2(currMB, (sColorPlane) (uv + 1)); // transform new intra DC
      }
    } //IS_I16MB

    update_qp(currMB, currSlice->qp);

    qp_per = vidParam->qp_per_matrix[ (currSlice->qp + vidParam->bitdepth_luma_qp_scale) ];
    qp_rem = vidParam->qp_rem_matrix[ (currSlice->qp + vidParam->bitdepth_luma_qp_scale) ];

    //init constants for every chroma qp offset
    qp_per_uv[uv] = vidParam->qp_per_matrix[ (currMB->qpc[uv] + vidParam->bitdepth_chroma_qp_scale) ];
    qp_rem_uv[uv] = vidParam->qp_rem_matrix[ (currMB->qpc[uv] + vidParam->bitdepth_chroma_qp_scale) ];
...

    {
      if (cbp)
      {
        ...
        {
          //======= 8x8 transform size & CABAC ========
          currMB->read_comp_coeff_8x8_CABAC (currMB, &currSE, (sColorPlane) (PLANE_U + uv));
        }
        else //4x4
        {
          currMB->read_comp_coeff_4x4_CABAC (currMB, &currSE, (sColorPlane) (PLANE_U + uv), InvLevelScale4x4,  qp_per_uv[uv], cbp);
        }
      }
    }
  }
...
void set_read_CBP_and_coeffs_cabac(sSlice* currSlice)
{
