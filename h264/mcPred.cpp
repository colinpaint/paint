...
namespace {
  ...
  void perform_mc_single_wp (sMacroBlock* mb, eColorPlane plane, sPicture* picture,
    ...
    if (mb->mbField && ((decoder->activePps->hasWeightedPred && (type == cSlice::eP || type == cSlice::eSP)) || 
                        (decoder->activePps->weightedBiPredIdc==1 && (type == cSlice::eB))))
      ref_idx_wp >>=1;
