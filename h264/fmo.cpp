...
namespace {
  //{{{
  void FmoGenerateType0MapUnitMap (cDecoder264* decoder, uint32_t PicSizeInMapUnits ) {
  ...
  void FmoGenerateType1MapUnitMap (cDecoder264* decoder, uint32_t PicSizeInMapUnits ) {
  ...
  void FmoGenerateType2MapUnitMap (cDecoder264* decoder, uint32_t PicSizeInMapUnits ) {
  ...
  void FmoGenerateType3MapUnitMap (cDecoder264* decoder, uint32_t PicSizeInMapUnits, cSlice* slice) {
  ...
  //}}}
  //{{{
  void FmoGenerateType4MapUnitMap (cDecoder264* decoder, uint32_t PicSizeInMapUnits, cSlice* slice) {
  ...
  void FmoGenerateType5MapUnitMap (cDecoder264* decoder, uint32_t PicSizeInMapUnits, cSlice* slice ) {

    cPps* pps = decoder->activePps;

    uint32_t mapUnitsInSliceGroup0 = imin ((pps->sliceGroupChangeRateMius1 + 1) * slice->sliceGroupChangeCycle, 
                                           PicSizeInMapUnits);
    uint32_t sizeOfUpperLeftGroup = pps->sliceGroupChangeDirectionFlag ? (PicSizeInMapUnits - mapUnitsInSliceGroup0) 
                                                                       : mapUnitsInSliceGroup0;

    uint32_t k = 0;
    for (uint32_t j = 0; j < decoder->coding.picWidthMbs; j++)
      for (uint32_t i = 0; i < decoder->coding.picHeightMapUnits; i++)
  ...
  //}}}
  //{{{
  void FmoGenerateType6MapUnitMap (cDecoder264* decoder, uint32_t PicSizeInMapUnits ) {
  ...
  //}}}
  //{{{
  int FmoGenerateMapUnitToSliceGroupMap (cDecoder264* decoder, cSlice* slice) {
  ...
  // Has to be called every time a new Picture Parameter Set is used
    ...
    uint32_t NumSliceGroupMapUnits = (sps->picHeightMapUnitsMinus1+1)* (sps->picWidthMbsMinus1+1);
  ...
  int FmoGenerateMbToSliceGroupMap (cDecoder264* decoder, cSlice *slice) {
