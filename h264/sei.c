...
//{{{
static void interpret_spare_pic (byte* payload, int size, VideoParameters* pVid )
{
  int x,y;
  Bitstream* buf;
  ...
  target_frame_num = read_ue_v ("SEI: target_frame_num", buf, &gDecoder->UsedBits);
  num_spare_pics = 1 + read_ue_v ("SEI: num_spare_pics_minus1", buf, &gDecoder->UsedBits);
  printf ("SEQ spare picture target_frame_num:%d num_spare_pics:%d\n",
          target_frame_num, num_spare_pics);

  get_mem3D (&map, num_spare_pics, pVid->height >> 4, pVid->width >> 4);
  for (int i = 0; i < num_spare_pics; i++) {
    ...
    delta_spare_frame_num = read_ue_v ("SEI: delta_spare_frame_num", buf, &gDecoder->UsedBits);
    SpareFrameNum = CandidateSpareFrameNum - delta_spare_frame_num;
    if (SpareFrameNum < 0 )
      SpareFrameNum = MAX_FN + SpareFrameNum;

    ref_area_indicator = read_ue_v ("SEI: ref_area_indicator", buf, &gDecoder->UsedBits);
    switch (ref_area_indicator) {
      //{{{
      case 0: // The whole frame can serve as spare picture
        for (y=0; y<pVid->height >> 4; y++)
          for (x=0; x<pVid->width >> 4; x++)
            map[i][y][x] = 0;
        break;
      //}}}
      //{{{
      case 1: // The map is not compressed
        for (y=0; y<pVid->height >> 4; y++)
          for (x=0; x<pVid->width >> 4; x++)
            map[i][y][x] = (byte) read_u_1("SEI: ref_mb_indicator", buf, &gDecoder->UsedBits);
        break;
      //}}}
      //{{{
      case 2: // The map is compressed
        ...
        for (m = 0; m < pVid->height >> 4; m++)
          for (n = 0; n < pVid->width >> 4; n++) {
            if (no_bit0 < 0)
              no_bit0 = read_ue_v ("SEI: zero_run_length", buf, &gDecoder->UsedBits);
            if (no_bit0>0)
              map[i][y][x] = (byte) bit0;
            else
              map[i][y][x] = (byte) bit1;
...
//{{{
static void interpret_subsequence_info (byte* payload, int size, VideoParameters* pVid )
{
  Bitstream* buf;
  ...
  sub_seq_layer_num        = read_ue_v("SEI: sub_seq_layer_num"       , buf, &gDecoder->UsedBits);
  sub_seq_id               = read_ue_v("SEI: sub_seq_id"              , buf, &gDecoder->UsedBits);
  first_ref_pic_flag       = read_u_1 ("SEI: first_ref_pic_flag"      , buf, &gDecoder->UsedBits);
  leading_non_ref_pic_flag = read_u_1 ("SEI: leading_non_ref_pic_flag", buf, &gDecoder->UsedBits);
  last_pic_flag            = read_u_1 ("SEI: last_pic_flag"           , buf, &gDecoder->UsedBits);
  sub_seq_frame_num_flag   = read_u_1 ("SEI: sub_seq_frame_num_flag"  , buf, &gDecoder->UsedBits);
  if (sub_seq_frame_num_flag)
    sub_seq_frame_num        = read_ue_v("SEI: sub_seq_frame_num"       , buf, &gDecoder->UsedBits);
...
//}}}
//{{{
static void interpret_subsequence_layer_characteristics_info (byte* payload, int size, VideoParameters* pVid )
{
  Bitstream* buf;
  ...
  num_sub_layers = 1 + read_ue_v("SEI: num_sub_layers_minus1", buf, &gDecoder->UsedBits);
  printf ("SEI Sub-sequence layer characteristics num_sub_layers_minus1 %ld\n", num_sub_layers - 1);
  for (int i = 0; i < num_sub_layers; i++) {
    accurate_statistics_flag = read_u_1 ("SEI: accurate_statistics_flag", buf, &gDecoder->UsedBits);
    average_bit_rate = read_u_v (16,"SEI: average_bit_rate", buf, &gDecoder->UsedBits);
    average_frame_rate = read_u_v (16,"SEI: average_frame_rate", buf, &gDecoder->UsedBits);
...
//}}}
//{{{
static void interpret_subsequence_characteristics_info (byte* payload, int size, VideoParameters* pVid )
{
  Bitstream* buf;
  int i;
  ...
  sub_seq_layer_num = read_ue_v("SEI: sub_seq_layer_num", buf, &gDecoder->UsedBits);
  sub_seq_id        = read_ue_v("SEI: sub_seq_id", buf, &gDecoder->UsedBits);
  duration_flag     = read_u_1 ("SEI: duration_flag", buf, &gDecoder->UsedBits);
  ...
  if ( duration_flag )
  {
    sub_seq_duration = read_u_v (32, "SEI: duration_flag", buf, &gDecoder->UsedBits);
    printf("sub_seq_duration = %ld\n", sub_seq_duration);
  }

  average_rate_flag = read_u_1 ("SEI: average_rate_flag", buf, &gDecoder->UsedBits);
  ...
  num_referenced_subseqs  = read_ue_v("SEI: num_referenced_subseqs", buf, &gDecoder->UsedBits);

  printf("num_referenced_subseqs = %d\n", num_referenced_subseqs);

  for (i=0; i<num_referenced_subseqs; i++)
  {
    ref_sub_seq_layer_num  = read_ue_v("SEI: ref_sub_seq_layer_num", buf, &gDecoder->UsedBits);
    ref_sub_seq_id         = read_ue_v("SEI: ref_sub_seq_id", buf, &gDecoder->UsedBits);
    ref_sub_seq_direction  = read_u_1 ("SEI: ref_sub_seq_direction", buf, &gDecoder->UsedBits);
...
//}}}
//{{{
static void interpret_scene_information (byte* payload, int size, VideoParameters* pVid )
{
  Bitstream* buf;
  ...
  scene_id              = read_ue_v("SEI: scene_id"             , buf, &gDecoder->UsedBits);
  scene_transition_type = read_ue_v("SEI: scene_transition_type", buf, &gDecoder->UsedBits);
  if ( scene_transition_type > 3 )
    second_scene_id     = read_ue_v("SEI: scene_transition_type", buf, &gDecoder->UsedBits);
...
//}}}
//{{{
static void interpret_filler_payload_info (byte* payload, int size, VideoParameters* pVid )
{
  int payload_cnt = 0;
...
//{{{
static void interpret_user_data_unregistered_info (byte* payload, int size, VideoParameters* pVid )
{
  int offset = 0;
  byte payload_byte;
...
//}}}
//{{{
static void interpret_user_data_registered_itu_t_t35_info (byte* payload, int size, VideoParameters* pVid ) {
...
//}}}
//{{{
static void interpret_reserved_info (byte* payload, int size, VideoParameters* pVid )
{
  int offset = 0;
  byte payload_byte;
...
//{{{
static void interpret_picture_timing_info (byte* payload, int size, VideoParameters* pVid ) {

  seq_parameter_set_rbsp_t *active_sps = pVid->active_sps;
  ...
  if (CpbDpbDelaysPresentFlag ) {
    ...
    if ((active_sps->vui_seq_parameters.nal_hrd_parameters_present_flag)||
      ...
      cpb_removal_delay = read_u_v(cpb_removal_len, "SEI: cpb_removal_delay" , buf, &gDecoder->UsedBits);
      dpb_output_delay = read_u_v(dpb_output_len,  "SEI: dpb_output_delay"  , buf, &gDecoder->UsedBits);
      if (kDebug) {
  ...
  if (pic_struct_present_flag) {
    pic_struct = read_u_v (4, "SEI: pic_struct" , buf, &gDecoder->UsedBits);
    //printf ("pic_struct = %d\n",pic_struct);
    switch (pic_struct) {
      case 0:
      case 1:
    ...
    if (kDebug)
      for (int i = 0; i < NumClockTs; i++) {
        ...
        clock_timestamp_flag = read_u_1("SEI: clock_timestamp_flag"  , buf, &gDecoder->UsedBits);
        printf ("clock_timestamp_flag = %d\n",clock_timestamp_flag);
        if (clock_timestamp_flag) {
          ct_type               = read_u_v(2, "SEI: ct_type"               , buf, &gDecoder->UsedBits);
          nuit_field_based_flag = read_u_1(   "SEI: nuit_field_based_flag" , buf, &gDecoder->UsedBits);
          counting_type         = read_u_v(5, "SEI: counting_type"         , buf, &gDecoder->UsedBits);
          full_timestamp_flag   = read_u_1(   "SEI: full_timestamp_flag"   , buf, &gDecoder->UsedBits);
          discontinuity_flag    = read_u_1(   "SEI: discontinuity_flag"    , buf, &gDecoder->UsedBits);
          cnt_dropped_flag      = read_u_1(   "SEI: cnt_dropped_flag"      , buf, &gDecoder->UsedBits);
          nframes               = read_u_v(8, "SEI: nframes"               , buf, &gDecoder->UsedBits);
          ...
          if (full_timestamp_flag) {
            seconds_value         = read_u_v(6, "SEI: seconds_value"   , buf, &gDecoder->UsedBits);
            minutes_value         = read_u_v(6, "SEI: minutes_value"   , buf, &gDecoder->UsedBits);
            hours_value           = read_u_v(5, "SEI: hours_value"     , buf, &gDecoder->UsedBits);
            printf("seconds_value = %d\n",seconds_value);
            printf("minutes_value = %d\n",minutes_value);
            printf("hours_value   = %d\n",hours_value);
            }
          else {
            seconds_flag          = read_u_1(   "SEI: seconds_flag" , buf, &gDecoder->UsedBits);
            printf ("seconds_flag = %d\n",seconds_flag);
            if (seconds_flag) {
              seconds_value         = read_u_v(6, "SEI: seconds_value"   , buf, &gDecoder->UsedBits);
              minutes_flag          = read_u_1(   "SEI: minutes_flag" , buf, &gDecoder->UsedBits);
              printf("seconds_value = %d\n",seconds_value);
              printf("minutes_flag  = %d\n",minutes_flag);
              if(minutes_flag) {
                minutes_value         = read_u_v(6, "SEI: minutes_value"   , buf, &gDecoder->UsedBits);
                hours_flag            = read_u_1(   "SEI: hours_flag" , buf, &gDecoder->UsedBits);
                printf("minutes_value = %d\n",minutes_value);
                printf("hours_flag    = %d\n",hours_flag);
                if(hours_flag) {
                  hours_value           = read_u_v(5, "SEI: hours_value"     , buf, &gDecoder->UsedBits);
...

            {
              ...
              if (time_offset_length)
                time_offset = read_i_v(time_offset_length, "SEI: time_offset"   , buf, &gDecoder->UsedBits);
              else
                time_offset = 0;
...
//}}}
//{{{
static void interpret_pan_scan_rect_info (byte* payload, int size, VideoParameters* pVid ) {
  ...
  int pan_scan_rect_id = read_ue_v ("SEI: pan_scan_rect_id", buf, &gDecoder->UsedBits);
  int pan_scan_rect_cancel_flag = read_u_1 ("SEI: pan_scan_rect_cancel_flag", buf, &gDecoder->UsedBits);
  if (!pan_scan_rect_cancel_flag) {
    int pan_scan_cnt_minus1 = read_ue_v ("SEI: pan_scan_cnt_minus1", buf, &gDecoder->UsedBits);
    for (int i = 0; i <= pan_scan_cnt_minus1; i++) {
      int pan_scan_rect_left_offset = read_se_v ("SEI: pan_scan_rect_left_offset", buf, &gDecoder->UsedBits);
      int pan_scan_rect_right_offset = read_se_v ("SEI: pan_scan_rect_right_offset", buf, &gDecoder->UsedBits);
      int pan_scan_rect_top_offset = read_se_v ("SEI: pan_scan_rect_top_offset", buf, &gDecoder->UsedBits);
      int pan_scan_rect_bottom_offset = read_se_v ("SEI: pan_scan_rect_bottom_offset", buf, &gDecoder->UsedBits);
      if (kDebug)
    ...
    int pan_scan_rect_repetition_period = read_ue_v ("SEI: pan_scan_rect_repetition_period", buf, &gDecoder->UsedBits);
    }
...
//}}}
//{{{
static void interpret_recovery_point_info (byte* payload, int size, VideoParameters* pVid ) {
  ...
  int recovery_frame_cnt = read_ue_v ("SEI: recovery_frame_cnt", buf, &gDecoder->UsedBits);
  int exact_match_flag = read_u_1 ("SEI: exact_match_flag", buf, &gDecoder->UsedBits);
  int broken_link_flag = read_u_1 ("SEI: broken_link_flag", buf, &gDecoder->UsedBits);
  int changing_slice_group_idc = read_u_v (2, "SEI: changing_slice_group_idc", buf, &gDecoder->UsedBits);

  pVid->recovery_point = 1;
  pVid->recovery_frame_cnt = recovery_frame_cnt;
  if (kDebug)
...
//}}}
//{{{
static void interpret_dec_ref_pic_marking_repetition_info (byte* payload, int size, VideoParameters* pVid, Slice *pSlice )
{
  ...
  original_idr_flag     = read_u_1 (    "SEI: original_idr_flag"    , buf, &gDecoder->UsedBits);
  original_frame_num    = read_ue_v(    "SEI: original_frame_num"   , buf, &gDecoder->UsedBits);

  if (!pVid->active_sps->frame_mbs_only_flag ) {
    original_field_pic_flag = read_u_1 ( "SEI: original_field_pic_flag", buf, &gDecoder->UsedBits);
    if ( original_field_pic_flag )
      original_bottom_field_flag = read_u_1 ( "SEI: original_bottom_field_flag", buf, &gDecoder->UsedBits);
    }
  ...
  old_idr_flag = pSlice->idr_flag; //pVid->idr_flag;

  old_no_output_of_prior_pics_flag = pSlice->no_output_of_prior_pics_flag; //pVid->no_output_of_prior_pics_flag;
  ...
  //pVid->idr_flag = original_idr_flag;
  pSlice->idr_flag = original_idr_flag;
  pSlice->dec_ref_pic_marking_buffer = NULL;
  dec_ref_pic_marking (pVid, buf, pSlice);
  //{{{  print out decoded values
  //if (pVid->idr_flag)
  //{
    //printf("no_output_of_prior_pics_flag = %d\n", pVid->no_output_of_prior_pics_flag);
    //printf("long_term_reference_flag     = %d\n", pVid->long_term_reference_flag);
  //}
  //else
  //{
    //printf("adaptive_ref_pic_buffering_flag  = %d\n", pVid->adaptive_ref_pic_buffering_flag);
    //if (pVid->adaptive_ref_pic_buffering_flag)
    //{
      //tmp_drpm=pVid->dec_ref_pic_marking_buffer;
      //while (tmp_drpm != NULL)
      //{
  ...
  pVid->no_output_of_prior_pics_flag = pSlice->no_output_of_prior_pics_flag;
...
//{{{
static void interpret_full_frame_freeze_info (byte* payload, int size, VideoParameters* pVid ) {
  ...
  full_frame_freeze_repetition_period = read_ue_v ("SEI: full_frame_freeze_repetition_period", buf, &gDecoder->UsedBits);
...
//}}}
//{{{
static void interpret_full_frame_freeze_release_info (byte* payload, int size, VideoParameters* pVid ) {
...
//}}}
//{{{
static void interpret_full_frame_snapshot_info (byte* payload, int size, VideoParameters* pVid ) {
  ...
  int snapshot_id = read_ue_v("SEI: snapshot_id", buf, &gDecoder->UsedBits);
...
//{{{
static void interpret_progressive_refinement_start_info (byte* payload, int size, VideoParameters* pVid )
{
  ...
  progressive_refinement_id   = read_ue_v("SEI: progressive_refinement_id"  , buf, &gDecoder->UsedBits);
  num_refinement_steps_minus1 = read_ue_v("SEI: num_refinement_steps_minus1", buf, &gDecoder->UsedBits);
...
//}}}
//{{{
static void interpret_progressive_refinement_end_info (byte* payload, int size, VideoParameters* pVid )
{
...
//{{{
static void interpret_motion_constrained_slice_group_set_info (byte* payload, int size, VideoParameters* pVid )
{
  ...
  num_slice_groups_minus1   = read_ue_v("SEI: num_slice_groups_minus1"  , buf, &gDecoder->UsedBits);
  ...
  for (i=0; i<=num_slice_groups_minus1;i++)
  {
    ...
    slice_group_id   = read_u_v (sliceGroupSize, "SEI: slice_group_id" , buf, &gDecoder->UsedBits);
    printf("slice_group_id            = %d\n", slice_group_id);
  }

  exact_match_flag   = read_u_1("SEI: exact_match_flag"  , buf, &gDecoder->UsedBits);
  pan_scan_rect_flag = read_u_1("SEI: pan_scan_rect_flag"  , buf, &gDecoder->UsedBits);
  ...
  {
    pan_scan_rect_id = read_ue_v("SEI: pan_scan_rect_id"  , buf, &gDecoder->UsedBits);
...
//}}}
//{{{
static void interpret_film_grain_characteristics_info (byte* payload, int size, VideoParameters* pVid )
{
  ...
  film_grain_characteristics_cancel_flag = read_u_1("SEI: film_grain_characteristics_cancel_flag", buf, &gDecoder->UsedBits);
  ...
  if(!film_grain_characteristics_cancel_flag)
  {
    ...
    model_id                                    = read_u_v(2, "SEI: model_id", buf, &gDecoder->UsedBits);
    separate_colour_description_present_flag    = read_u_1("SEI: separate_colour_description_present_flag", buf, &gDecoder->UsedBits);
    printf("model_id = %d\n", model_id);
    ...
    {
      film_grain_bit_depth_luma_minus8          = read_u_v(3, "SEI: film_grain_bit_depth_luma_minus8", buf, &gDecoder->UsedBits);
      film_grain_bit_depth_chroma_minus8        = read_u_v(3, "SEI: film_grain_bit_depth_chroma_minus8", buf, &gDecoder->UsedBits);
      film_grain_full_range_flag                = read_u_v(1, "SEI: film_grain_full_range_flag", buf, &gDecoder->UsedBits);
      film_grain_colour_primaries               = read_u_v(8, "SEI: film_grain_colour_primaries", buf, &gDecoder->UsedBits);
      film_grain_transfer_characteristics       = read_u_v(8, "SEI: film_grain_transfer_characteristics", buf, &gDecoder->UsedBits);
      film_grain_matrix_coefficients            = read_u_v(8, "SEI: film_grain_matrix_coefficients", buf, &gDecoder->UsedBits);
    ...
    blending_mode_id                            = read_u_v(2, "SEI: blending_mode_id", buf, &gDecoder->UsedBits);
    log2_scale_factor                           = read_u_v(4, "SEI: log2_scale_factor", buf, &gDecoder->UsedBits);
    printf("blending_mode_id = %d\n", blending_mode_id);
    printf("log2_scale_factor = %d\n", log2_scale_factor);
    for (c = 0; c < 3; c ++)
    {
      comp_model_present_flag[c]                = read_u_1("SEI: comp_model_present_flag", buf, &gDecoder->UsedBits);
      printf("comp_model_present_flag = %d\n", comp_model_present_flag[c]);
    }
    for (c = 0; c < 3; c ++)
      if (comp_model_present_flag[c])
      {
        num_intensity_intervals_minus1          = read_u_v(8, "SEI: num_intensity_intervals_minus1", buf, &gDecoder->UsedBits);
        num_model_values_minus1                 = read_u_v(3, "SEI: num_model_values_minus1", buf, &gDecoder->UsedBits);
        ...
        {
          intensity_interval_lower_bound        = read_u_v(8, "SEI: intensity_interval_lower_bound", buf, &gDecoder->UsedBits);
          intensity_interval_upper_bound        = read_u_v(8, "SEI: intensity_interval_upper_bound", buf, &gDecoder->UsedBits);
          ...
          {
            comp_model_value                    = read_se_v("SEI: comp_model_value", buf, &gDecoder->UsedBits);
            printf("comp_model_value = %d\n", comp_model_value);
          }
        }
      }
    film_grain_characteristics_repetition_period = read_ue_v("SEI: film_grain_characteristics_repetition_period", buf, &gDecoder->UsedBits);
...
//}}}
//{{{
static void interpret_deblocking_filter_display_preference_info (byte* payload, int size, VideoParameters* pVid )
{
  ...
  deblocking_display_preference_cancel_flag             = read_u_1("SEI: deblocking_display_preference_cancel_flag", buf, &gDecoder->UsedBits);
  ...
  if (!deblocking_display_preference_cancel_flag) {
    display_prior_to_deblocking_preferred_flag            = read_u_1("SEI: display_prior_to_deblocking_preferred_flag", buf, &gDecoder->UsedBits);
    dec_frame_buffering_constraint_flag                   = read_u_1("SEI: dec_frame_buffering_constraint_flag", buf, &gDecoder->UsedBits);
    deblocking_display_preference_repetition_period       = read_ue_v("SEI: deblocking_display_preference_repetition_period", buf, &gDecoder->UsedBits);
...
//}}}
//{{{
static void interpret_stereo_video_info_info (byte* payload, int size, VideoParameters* pVid )
{
  ...
  field_views_flags = read_u_1 ("SEI: field_views_flags", buf, &gDecoder->UsedBits);
  printf("field_views_flags = %d\n", field_views_flags);
  if (field_views_flags) {
    top_field_is_left_view_flag = read_u_1 ("SEI: top_field_is_left_view_flag", buf, &gDecoder->UsedBits);
    printf("top_field_is_left_view_flag = %d\n", top_field_is_left_view_flag);
    }
  else {
    current_frame_is_left_view_flag = read_u_1 ("SEI: current_frame_is_left_view_flag", buf, &gDecoder->UsedBits);
    next_frame_is_second_view_flag = read_u_1 ("SEI: next_frame_is_second_view_flag", buf, &gDecoder->UsedBits);
  ...
  left_view_self_contained_flag = read_u_1 ("SEI: left_view_self_contained_flag", buf, &gDecoder->UsedBits);
  right_view_self_contained_flag = read_u_1 ("SEI: right_view_self_contained_flag", buf, &gDecoder->UsedBits);
...
//}}}
//{{{
static void interpret_buffering_period_info (byte* payload, int size, VideoParameters* pVid ) {
  ...
  int seq_parameter_set_id = read_ue_v("SEI: seq_parameter_set_id"  , buf, &gDecoder->UsedBits);
  seq_parameter_set_rbsp_t* sps = &pVid->SeqParSet[seq_parameter_set_id];
  ...
  if (sps->vui_parameters_present_flag) {
    ...
    if (sps->vui_seq_parameters.nal_hrd_parameters_present_flag) {
      for (unsigned k = 0; k < sps->vui_seq_parameters.nal_hrd_parameters.cpb_cnt_minus1+1; k++) {
        initial_cpb_removal_delay = read_u_v(sps->vui_seq_parameters.nal_hrd_parameters.initial_cpb_removal_delay_length_minus1+1, "SEI: initial_cpb_removal_delay"        , buf, &gDecoder->UsedBits);
        initial_cpb_removal_delay_offset = read_u_v(sps->vui_seq_parameters.nal_hrd_parameters.initial_cpb_removal_delay_length_minus1+1, "SEI: initial_cpb_removal_delay_offset" , buf, &gDecoder->UsedBits);
    ...
    if (sps->vui_seq_parameters.vcl_hrd_parameters_present_flag) {
      for (unsigned k = 0; k < sps->vui_seq_parameters.vcl_hrd_parameters.cpb_cnt_minus1+1; k++) {
        initial_cpb_removal_delay = read_u_v(sps->vui_seq_parameters.vcl_hrd_parameters.initial_cpb_removal_delay_length_minus1+1, "SEI: initial_cpb_removal_delay"        , buf, &gDecoder->UsedBits);
        initial_cpb_removal_delay_offset = read_u_v(sps->vui_seq_parameters.vcl_hrd_parameters.initial_cpb_removal_delay_length_minus1+1, "SEI: initial_cpb_removal_delay_offset" , buf, &gDecoder->UsedBits);
...
//}}}
//{{{
static void interpret_frame_packing_arrangement_info (byte* payload, int size, VideoParameters* pVid )
{
  ...
  if ( seiFramePackingArrangement.frame_packing_arrangement_cancel_flag == FALSE )
  {
    ...
    seiFramePackingArrangement.frame_packing_arrangement_reserved_byte = (unsigned char)read_u_v( 8, "SEI: frame_packing_arrangement_reserved_byte", buf, &gDecoder->UsedBits );
    seiFramePackingArrangement.frame_packing_arrangement_repetition_period = (unsigned int)read_ue_v( "SEI: frame_packing_arrangement_repetition_period", buf, &gDecoder->UsedBits );
  ...
  seiFramePackingArrangement.frame_packing_arrangement_extension_flag = read_u_1( "SEI: frame_packing_arrangement_extension_flag", buf, &gDecoder->UsedBits );
...
//{{{
static void interpret_post_filter_hints_info (byte* payload, int size, VideoParameters* pVid )
{
  ...
  filter_hint_size_y = read_ue_v("SEI: filter_hint_size_y", buf, &gDecoder->UsedBits); // interpret post-filter hint SEI here
  filter_hint_size_x = read_ue_v("SEI: filter_hint_size_x", buf, &gDecoder->UsedBits); // interpret post-filter hint SEI here
  filter_hint_type   = read_u_v(2, "SEI: filter_hint_type", buf, &gDecoder->UsedBits); // interpret post-filter hint SEI here
  ...
  for (color_component = 0; color_component < 3; color_component ++)
    for (cy = 0; cy < filter_hint_size_y; cy ++)
      for (cx = 0; cx < filter_hint_size_x; cx ++)
        filter_hint[color_component][cy][cx] = read_se_v("SEI: filter_hint", buf, &gDecoder->UsedBits); // interpret post-filter hint SEI here

  additional_extension_flag = read_u_1("SEI: additional_extension_flag", buf, &gDecoder->UsedBits); // interpret post-filter hint SEI here
...
//}}}
//{{{
static void interpret_green_metadata_info (byte* payload, int size, VideoParameters* pVid )
{
  ...
  seiGreenMetadataInfo.green_metadata_type=(unsigned char)read_u_v(8, "SEI: green_metadata_type", buf, &gDecoder->UsedBits );
  printf("green_metadata_type                 = %d\n", seiGreenMetadataInfo.green_metadata_type);
  if (seiGreenMetadataInfo.green_metadata_type == 0) {
      seiGreenMetadataInfo.period_type=(unsigned char)read_u_v(8, "SEI: green_metadata_period_type", buf, &gDecoder->UsedBits );
    printf("green_metadata_period_type     = %d\n", seiGreenMetadataInfo.period_type);

    if (seiGreenMetadataInfo.period_type == 2) {
      seiGreenMetadataInfo.num_seconds = (unsigned short)read_u_v(16, "SEI: green_metadata_num_seconds", buf, &gDecoder->UsedBits );
      printf("green_metadata_num_seconds      = %d\n", seiGreenMetadataInfo.num_seconds);
      }
    else if (seiGreenMetadataInfo.period_type == 3) {
      seiGreenMetadataInfo.num_pictures = (unsigned short)read_u_v(16, "SEI: green_metadata_num_pictures", buf, &gDecoder->UsedBits );
      printf("green_metadata_num_pictures      = %d\n", seiGreenMetadataInfo.num_pictures);
      }

    seiGreenMetadataInfo.percent_non_zero_macroblocks=(unsigned char)read_u_v(8, "SEI: percent_non_zero_macroblocks", buf, &gDecoder->UsedBits );
    seiGreenMetadataInfo.percent_intra_coded_macroblocks=(unsigned char)read_u_v(8, "SEI: percent_intra_coded_macroblocks", buf, &gDecoder->UsedBits );
    seiGreenMetadataInfo.percent_six_tap_filtering=(unsigned char)read_u_v(8, "SEI: percent_six_tap_filtering", buf, &gDecoder->UsedBits );
    seiGreenMetadataInfo.percent_alpha_point_deblocking_instance=(unsigned char)read_u_v(8, "SEI: percent_alpha_point_deblocking_instance", buf, &gDecoder->UsedBits );
    ...
    seiGreenMetadataInfo.xsd_metric_type=(unsigned char)read_u_v(8, "SEI: xsd_metric_type", buf, &gDecoder->UsedBits );
    seiGreenMetadataInfo.xsd_metric_value=(unsigned short)read_u_v(16, "SEI: xsd_metric_value", buf, &gDecoder->UsedBits );
...
//{{{
void InterpretSEIMessage (byte* msg, int size, VideoParameters* pVid, Slice *pSlice) {
  ...
  do {
    ...
    switch (payload_type) {
      case  SEI_BUFFERING_PERIOD:
        interpret_buffering_period_info (msg+offset, payload_size, pVid ); break;
      case  SEI_PIC_TIMING:
        interpret_picture_timing_info (msg+offset, payload_size, pVid ); break;
      case  SEI_PAN_SCAN_RECT:
        interpret_pan_scan_rect_info (msg+offset, payload_size, pVid ); break;
      case  SEI_FILLER_PAYLOAD:
        interpret_filler_payload_info (msg+offset, payload_size, pVid ); break;
      case  SEI_USER_DATA_REGISTERED_ITU_T_T35:
        interpret_user_data_registered_itu_t_t35_info (msg+offset, payload_size, pVid ); break;
      case  SEI_USER_DATA_UNREGISTERED:
        interpret_user_data_unregistered_info (msg+offset, payload_size, pVid ); break;
      case  SEI_RECOVERY_POINT:
        interpret_recovery_point_info (msg+offset, payload_size, pVid ); break;
      case  SEI_DEC_REF_PIC_MARKING_REPETITION:
        interpret_dec_ref_pic_marking_repetition_info (msg+offset, payload_size, pVid, pSlice ); break;
      case  SEI_SPARE_PIC:
        interpret_spare_pic (msg+offset, payload_size, pVid ); break;
      case  SEI_SCENE_INFO:
        interpret_scene_information (msg+offset, payload_size, pVid ); break;
      case  SEI_SUB_SEQ_INFO:
        interpret_subsequence_info (msg+offset, payload_size, pVid ); break;
      case  SEI_SUB_SEQ_LAYER_CHARACTERISTICS:
        interpret_subsequence_layer_characteristics_info (msg+offset, payload_size, pVid ); break;
      case  SEI_SUB_SEQ_CHARACTERISTICS:
        interpret_subsequence_characteristics_info (msg+offset, payload_size, pVid ); break;
      case  SEI_FULL_FRAME_FREEZE:
        interpret_full_frame_freeze_info (msg+offset, payload_size, pVid ); break;
      case  SEI_FULL_FRAME_FREEZE_RELEASE:
        interpret_full_frame_freeze_release_info (msg+offset, payload_size, pVid ); break;
      case  SEI_FULL_FRAME_SNAPSHOT:
        interpret_full_frame_snapshot_info (msg+offset, payload_size, pVid ); break;
      case  SEI_PROGRESSIVE_REFINEMENT_SEGMENT_START:
        interpret_progressive_refinement_start_info (msg+offset, payload_size, pVid ); break;
      case  SEI_PROGRESSIVE_REFINEMENT_SEGMENT_END:
        interpret_progressive_refinement_end_info (msg+offset, payload_size, pVid ); break;
      case  SEI_MOTION_CONSTRAINED_SLICE_GROUP_SET:
        interpret_motion_constrained_slice_group_set_info (msg+offset, payload_size, pVid ); break;
      case  SEI_FILM_GRAIN_CHARACTERISTICS:
        interpret_film_grain_characteristics_info (msg+offset, payload_size, pVid ); break;
      case  SEI_DEBLOCKING_FILTER_DISPLAY_PREFERENCE:
        interpret_deblocking_filter_display_preference_info (msg+offset, payload_size, pVid ); break;
      case  SEI_STEREO_VIDEO_INFO:
        interpret_stereo_video_info_info  (msg+offset, payload_size, pVid ); break;
      case  SEI_POST_FILTER_HINTS:
        interpret_post_filter_hints_info (msg+offset, payload_size, pVid ); break;
      case  SEI_FRAME_PACKING_ARRANGEMENT:
        interpret_frame_packing_arrangement_info (msg+offset, payload_size, pVid ); break;
      case  SEI_GREEN_METADATA:
        interpret_green_metadata_info (msg+offset, payload_size, pVid ); break;
      default:
        interpret_reserved_info (msg+offset, payload_size, pVid ); break;
      }
    offset += payload_size;
