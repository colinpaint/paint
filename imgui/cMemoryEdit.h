...
namespace {
  //{{{
  void* endianessCopyBigEndian (void* dst, void* src, size_t s, int is_little_endian) {
  ...
  //}}}
  //{{{
  void* endianessCopyLittleEndian (void* dst, void* src, size_t s, int is_little_endian) {
  ...
  //}}}
  void* (*fp)(void*, void*, size_t, int) = nullptr;
  }
...
public:
  ...
  cMemoryEdit() {
    ...
    mReadOnly = false;
    mCols = 16;
    mOptShowOptions = true;
    mOptShowDataPreview = false;
    mOptShowHexII = false;
    mOptShowAscii = true;
    mOptGreyOutZeroes = true;
    mOptUpperCaseHex = true;
    mOptMidColsCount = 8;
    mOptAddrDigitsCount = 0;
    mOptFooterExtraHeight = 0.0f;
    mHighlightColor = IM_COL32(255, 255, 255, 50);
    mReadFn = NULL;
    mWriteFn = NULL;
    mHighlightFn = NULL;

    // State/Internals
    mContentsWidthChanged = false;
    mDataPreviewAddr = mDataEditingAddr = (size_t)-1;
    mDataEditingTakeFocus = false;
    memset(mDataInputBuf, 0, sizeof(mDataInputBuf));
    memset(mAddrInputBuf, 0, sizeof(mAddrInputBuf));
    mGotoAddr = (size_t)-1;
    mHighlightMin = mHighlightMax = (size_t)-1;
    mPreviewEndianess = 0;
    mPreviewDataType = ImGuiDataType_S32;
    }
  //}}}

  //{{{
  void drawWindow (const char* title, void* mem_data, size_t memSize, size_t baseDisplayAddress) {
  // Standalone Memory Editor window
    ...
    if (ImGui::Begin (title, &mOpen, ImGuiWindowFlags_NoScrollbar)) {
      ...
      drawContents(mem_data, memSize, baseDisplayAddress);
  ...
  //}}}
  //{{{
  void drawContents (void* mem_data_void, size_t memSize, size_t baseDisplayAddress) {
    ...
    size_t preview_data_type_size = mOptShowDataPreview ? dataTypeGetSize (mPreviewDataType) : 0;
    ...
    for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) {
      ...
      ImGui::Text (format_address, s.AddrDigitsCount, baseDisplayAddress + addr);

      // Draw Hexadecimal
      for (int n = 0; n < mCols && addr < memSize; n++, addr++) {
        float byte_pos_x = s.PosHexStart + s.HexCellWidth * n;
        if (mOptMidColsCount > 0)
          byte_pos_x += (float)(n / mOptMidColsCount) * s.SpacingBetweenMidCols;
        ImGui::SameLine(byte_pos_x);

        // Draw highlight
        bool is_highlight_from_user_range = (addr >= mHighlightMin && addr < mHighlightMax);
        bool is_highlight_from_user_func = (mHighlightFn && mHighlightFn(mem_data, addr));
        ...
        if (is_highlight_from_user_range || is_highlight_from_user_func || is_highlight_from_preview) {
          ...
          bool is_next_byte_highlighted =  (addr + 1 < memSize) && ((mHighlightMax != (size_t)-1 && addr + 1 < mHighlightMax) || (mHighlightFn && mHighlightFn(mem_data, addr + 1)));
          if (is_next_byte_highlighted || (n + 1 == mCols)) {
            highlight_width = s.HexCellWidth;
            if (mOptMidColsCount > 0 && n > 0 && (n + 1) < mCols && ((n + 1) % mOptMidColsCount) == 0)
              highlight_width += s.SpacingBetweenMidCols;
            }
          draw_list->AddRectFilled (pos, ImVec2(pos.x + highlight_width, pos.y + s.LineHeight), mHighlightColor);
          }
        ...
        if (mDataEditingAddr == addr) {
          ...
          if (mDataEditingTakeFocus) {
              ...
              sprintf (mAddrInputBuf, format_data, s.AddrDigitsCount, baseDisplayAddress + addr);
              sprintf (mDataInputBuf, format_byte, mReadFn ? mReadFn(mem_data, addr) : mem_data[addr]);
          }
        struct UserData {
          //{{{
        ...
        sprintf (user_data.CurrentBufOverwrite, format_byte, mReadFn ? mReadFn(mem_data, addr) : mem_data[addr]);
        ...
        if (data_write && sscanf(mDataInputBuf, "%X", &data_input_value) == 1) {
          if (mWriteFn)
            mWriteFn(mem_data, addr, (ImU8)data_input_value);
          else
        ...
        ImU8 b = mReadFn ? mReadFn (mem_data, addr) : mem_data[addr];
    ...
    if (mOptShowAscii) {
      ...
      if (ImGui::InvisibleButton("ascii", ImVec2(s.PosAsciiEnd - s.PosAsciiStart, s.LineHeight))) {
        mDataEditingAddr = mDataPreviewAddr = addr + (size_t)((ImGui::GetIO().MousePos.x - pos.x) / s.GlyphWidth);
        mDataEditingTakeFocus = true;
        }
      ImGui::PopID();
      for (int n = 0; n < mCols && addr < memSize; n++, addr++) {
        if (addr == mDataEditingAddr) {
          ...
          unsigned char c = mReadFn ? mReadFn(mem_data, addr) : mem_data[addr];
    ...
    if (mOptShowOptions) {
      //{{{  draw options
      ImGui::Separator();
      drawOptionsLine (s, mem_data, memSize, baseDisplayAddress);
      }
      //}}}
    ...
    if (lock_show_data_preview) {
      //{{{  show preview
      ImGui::Separator();
      drawPreviewLine (s, mem_data, memSize, baseDisplayAddress);
      }
      //}}}
    }
  //}}}
...
private:
  ...
  void calcSizes (sSizes& s, size_t memSize, size_t baseDisplayAddress) {
    ...
    s.AddrDigitsCount = mOptAddrDigitsCount;
    ...
    s.PosHexEnd = s.PosHexStart + (s.HexCellWidth * mCols);
    s.PosAsciiStart = s.PosAsciiEnd = s.PosHexEnd;

    if (mOptShowAscii) {
      s.PosAsciiStart = s.PosHexEnd + s.GlyphWidth * 1;
      if (mOptMidColsCount > 0)
        s.PosAsciiStart += (float)((mCols + mOptMidColsCount - 1) / mOptMidColsCount) * s.SpacingBetweenMidCols;
      s.PosAsciiEnd = s.PosAsciiStart + mCols * s.GlyphWidth;
      }
  ...
  void drawOptionsLine (const sSizes& s, void* memData, size_t memSize, size_t baseDisplayAddress) {
    ...
    ImGui::SameLine();
    ImGui::Text (format_range, s.AddrDigitsCount, baseDisplayAddress, s.AddrDigitsCount, baseDisplayAddress + memSize - 1);
    ImGui::SameLine();
  ...
  //}}}
  //{{{
  void drawPreviewLine (const sSizes& s, void* memData_void, size_t memSize, size_t baseDisplayAddress) {

    IM_UNUSED(baseDisplayAddress);
    ImU8* memData = (ImU8*)memData_void;
    ...
    if (ImGui::BeginCombo("##combo_type", dataTypeGetDesc(mPreviewDataType), ImGuiComboFlags_HeightLargest)) {
      for (int n = 0; n < ImGuiDataType_COUNT; n++)
        if (ImGui::Selectable (dataTypeGetDesc((ImGuiDataType)n), mPreviewDataType == n))
  ...
  //}}}
  //{{{
  const char* dataFormatGetDesc (eDataFormat data_format) const {
  ...
  void* endianessCopy (void* dst, void* src, size_t size) const {
    ...
    return fp (dst, src, size, mPreviewEndianess);
    }
  //}}}
  //{{{
  ...
  bool  mReadOnly;                                   // = false  disable any editing.
  int   mCols;                                       // = 16     number of columns to display.
  ...
  ImU32 mHighlightColor;                             //          background color of highlighted bytes.
  ImU8  (*mReadFn)(const ImU8* data, size_t off);    // = 0      optional handler to read bytes.
  void  (*mWriteFn)(ImU8* data, size_t off, ImU8 d); // = 0      optional handler to write bytes.
  bool  (*mHighlightFn)(const ImU8* data, size_t off);//= 0      optional handler to return Highlight property (to support non-contiguous highlighting).

  bool    mContentsWidthChanged;
  size_t  mDataPreviewAddr;
  size_t  mDataEditingAddr;
  bool    mDataEditingTakeFocus;
  char    mDataInputBuf[32];
  char    mAddrInputBuf[32];
  size_t  mGotoAddr;
  size_t  mHighlightMin;
  size_t  mHighlightMax;
  int     mPreviewEndianess;
  ImGuiDataType mPreviewDataType;
  };
